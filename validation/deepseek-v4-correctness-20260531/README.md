...
Strict official/reference logprob parity is still recorded as FAIL. Full mini-checkpoint external reference one-step train parity remains `MISSING_INPUT` only for the stronger monolithic-reference claim: SFT loss and one-layer score-routed MoE are now independently covered, but we have not rewritten the entire 4-layer checkpoint, EP=8 routed MoE, backward, and update as a single independent PyTorch reference. The mini-checkpoint training correctness gate itself is now PASS under the declared BF16 tolerance.
